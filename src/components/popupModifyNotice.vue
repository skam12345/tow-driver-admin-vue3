  ...
  <div class="popup_cont">
    <div class="popup_contents_cont">
      ...
      <div class="popup_contents">
        <div class="popup_contents_top">공지사항 수정</div>
        ...
        <div class="popup_contents_bottom">
          <div class="btn_cont">
            ...
            <div class="bottom_btns_add" @click="modiftNotice()">수정</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const ndaSeqNo = ref(props.data.ndaSeqNo);
    const aiaAdminId = ref(props.data.aiaAdminId);
    const ndaContents = ref(props.data.ndaContents);
    const ndaIsYn = ref(props.data.ndaIsYn); // 노출 여부
    const ndaNoticeType = ref(props.data.ndaNoticeType); // 공지 구분
    const ndaTitle = ref(props.data.ndaTitle);
    const ndaWriteDate = ref(props.data.ndaWriteDate);
    const ndaSortNo = ref(props.data.ndaSortNo);
    ...
    const modiftNotice = () => {
      const modified = confirm("공지사항을 수정 하시겠습니까?");
      if (!modified) return;
      try {
        const val = validate();
        if (val) {
          axios
            .post("https://bie.autogram.co.kr/api/v1/notice/modify", {
              ndaSeqNo: ndaSeqNo.value,
              ...
              ndaWriteDate: ndaWriteDate.value,
            })
            .then((res) => {
              console.log(res);
