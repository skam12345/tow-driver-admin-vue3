  ...
  <div class="popup_cont">
    <div class="popup_contents_cont">
      ...
      <div class="popup_contents">
        <div class="popup_contents_top">단속 대상 차량 상세</div>
        <div class="popup_contents_body">
          ...
          <div class="popup_contents_body_input_cont">
            <div
              class="popup_contents_body_input_pw input_box2 date"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const deleteImgToS3Complete = ref();
    // s3 인증
    ...
    const selectFile = async (e, change) => {
      ...
      imgPreviewsWithoutEmpty.value = imgPreviews.value.filter(isImage);
      ...
      if (e.dataTransfer) {
        ...
        if (e.target.files.length > 4 && change === "change") {
          ...
          // 빈 값인 사진들 제거하고 사진 추가
          console.log(imgPreviews.value);
          imgPreviews.value = imgPreviewsWithoutEmpty.value;
        }
    ...
    const deleteImg = (e, info, index) => {
      ...
      if (val) {
        ...
        imgPreviews.value.push(emptyImgData);
        deletedImgIndex.value.push({ fileName: fileName });
        switch (fileName) {
          case "0":
            vdaImg01Path.value = "remove";
            break;
          case "1":
    ...
    const uploadImg = async () => {
      ...
      if (deletedImgIndex.value.length > 0 && imgFiles.value.length > 0) {
        ...
        // 삭제 없이 업로드만 할때
        console.log("삭제 안하고 업로드만");
        let array = [];
        for (let i = 0; i < imagePathFromS3.value.length; i++) {
          array.push(imagePathFromS3.value[i].fileName);
        }
        console.log(array);
        let answer = [];
        for (let i = 0; i < 4; i++) {
          if (!array.includes(`${i}`)) answer.push(`${i}`);
        }
        console.log(answer);
        for (let i = 0; i < imgFiles.value.length; i++) {
          // s3 upload
          ...
          switch (answer[i]) {
            case "0":
              vdaImg01Path.value = `${imagePathForDB}/${answer[i]}`;
              console.log(vdaImg01Path.value);
              break;
            case "1":
              vdaImg02Path.value = `${imagePathForDB}/${answer[i]}`;
              console.log(vdaImg02Path.value);
              break;
            case "2":
              vdaImg03Path.value = `${imagePathForDB}/${answer[i]}`;
              console.log(vdaImg03Path.value);
              break;
            case "3":
              vdaImg04Path.value = `${imagePathForDB}/${answer[i]}`;
              console.log(vdaImg04Path.value);
              break;
          }
        }
